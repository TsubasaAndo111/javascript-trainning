    ...
    case "<": // RedirCmd
      {
        ...
        const input = fs.createReadStream(cmd.file);
        await runcmd(cmd.cmd, input, stdout);
      }
      break;
...
async function main() {
  ...
  for await (const rawLine of rl) {
    ...
    if (line.startsWith("cd ")) {
      ...
      const cmd = parsecmd(line); // 入力のパース
      try {
        rl.pause(); // 入力受付を一時停止
        await runcmd(cmd); // パース結果を実行
      } catch (err) {
        console.error(err);
      } finally {
        rl.resume();
